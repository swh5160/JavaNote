...
<font color=goal>@Transactional注解书写位置：</font>
...
说明：<font color=red>可以在application.yml配置文件中开启事务管理日志，这样就可以在控制看到和事务相关的日志信息了</font>
...
<font color=red>通过以上测试可以得出一个结论：**spring默认情况下，只有出现RuntimeException(运行时异常)才会回滚事务**。</font>
...
我们通过AOP入门程序完成了业务方法执行耗时的统计，那其实AOP的功能远不止于此，常见的<font color=goal>**应用场景如下**：</font>
...
<font color=gold>通过入门程序，我们也应该感受到了AOP面向切面编程的一些优势：</font>
...
![image-20230331151250361](assets/image-20230331151250361.png)

###### aop底层代码执行流程

![image-20230331152135351](assets/image-20230331152135351.png)

![image-20230331152104496](assets/image-20230331152104496.png)
...
<font color=chartreuse>Spring中AOP的通知类型：</font>
...
<font color=cyan>在使用通知时的注意事项：</font>
...
<font color= deeppink>Spring提供了@PointCut注解，该注解的作用是将公共的切入点表达式抽取出来，需要用到时引用该切入点表达式即可。</font>
...
<font color= darkorange>通过以上程序运行可以看出在不同切面类中，默认按照切面类的类名字母排序：</font>
...
<font color=gold>如果我们想控制通知的执行顺序有两种方式：</font>

1. 修改切面类的类名（这种方式非常繁琐、而且不便管理）
2. 使用Spring提供的@Order注解(***<font color=red>数字越小先执行</font>***)
...
<font color= chartreuse>execution主要根据方法的返回值、包名、类名、方法名、方法参数等信息来匹配，语法为：</font>
...
<font color= chartreuse>切入点表达式的语法规则：</font>
...
<font color= chartreuse>切入点表达式的书写建议：</font>
...
<font color= deeppink>到此我们两种常见的切入点表达式我已经介绍完了。</font>

- execution切入点表达式
  - 根据我们所指定的方法的描述信息来匹配切入点方法，这种方式也是最为常用的一种方式
  - 如果我们要匹配的切入点方法的方法名不规则，或者有一些比较特殊的需求，通过execution切入点表达式描述比较繁琐
- annotation 切入点表达式
  - 基于注解的方式来匹配切入点方法。这种方式虽然多一步操作，我们需要自定义一个注解，但是相对来比较灵活。我们需要匹配哪个方法，就在方法上加上对应的注解就可以了(<font color=red>**细节**：自定义切面注解放在实现类方法上</font>)
...
<font color=red>在Spring中用JoinPoint抽象了连接点，用它可以获得方法执行时的相关信息，如目标类名、方法名、方法参数等。</font>
...
- 准备工作
  1. 引入AOP的起步依赖
  2. 导入资料中准备好的数据库表结构，并引入对应的实体类、Mapper接口
- 编码实现
  1. 自定义注解@Log
  2. 定义切面类，完成记录操作日志的逻辑

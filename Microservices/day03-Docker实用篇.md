...
在浏览器访问自己的虚拟机地址，例如我的是：http://47.120.39.42，即可看到结果：
...
2）创建目录/tmp/mysql/conf，将课前资料提供的hmy.cnf文件上传到/tmp/mysql/conf
...
最后访问 http://47.120.39.42:8090/hello/count，其中的ip改成你的虚拟机ip
...
3. 基础镜像可以是基本操作系统，如Ubuntu。也可以是其他人制作好的镜像，例如：openjdk:8
...
# 添加内容：
"insecure-registries":["http://47.120.39.42:8080"]
# 重加载
systemctl daemon-reload
# 重启docker
systemctl restart docker
...
① 重新tag本地镜像，名称前缀为私有仓库的地址：47.120.39.42:8080/

 ```sh
docker tag nginx:latest 47.120.39.42:8080/nginx:1.0 
 ```
...
```sh
docker push 47.120.39.42:8080/nginx:1.0 
```
...
```sh
docker pull 47.120.39.42:8080/nginx:1.0 
```
